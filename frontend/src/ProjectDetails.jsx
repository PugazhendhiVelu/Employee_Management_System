...
const StatusDot = styled(Box)(({ status }) => ({
  ...
  height: '10px',
  borderRadius: '50%',
  backgroundColor: status === 'Ongoing' ? 'green' : 'red',
  marginRight: '8px',
}));
...
export default function ProjectDetails() {
  ...
  useEffect(() => {
    if (projectId) {
      axios.get(`http://localhost:5000/getProject/id/${projectId}`)
        .then(response => {
  ...
  const handleSave = () => {
    axios.put(`http://localhost:5000/editProject/editbyid/${projectId}`, editedProject)
      .then(() => {
  ...
  const handleStatusClose = (status) => {
    if (status) {
      axios.put(`http://localhost:5000/editProject/id/${projectId}`, { ...editedProject, status })
        .then(() => {
